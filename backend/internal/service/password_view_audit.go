...
func PasswordViewAuditQuery(ctx *gin.Context) (*gorm.DB, error) {
	...
	query := db.GetDB().WithContext(ctx.Request.Context()).Table("pam_access_audit AS audit").
		...
		Joins("LEFT JOIN account ON audit.target_kind = ? AND account.id = audit.target_id AND (? OR audit.outcome = ?)", model.PAMOwnerAccount, acl.IsAdmin(operator), "succeeded").
		Joins("LEFT JOIN gateway ON audit.target_kind = ? AND gateway.id = audit.target_id AND (? OR audit.outcome = ?)", model.PAMOwnerGateway, acl.IsAdmin(operator), "succeeded").
